...
def get_datasets(data, label_col, tokenize_function, train_size = 0.8, sampling_strategy="oversampling", oversampling_probs=None, logger=None, noise_ratio=None, noisy_eval=False):
    ...
    logger.info(f"positive freq in original train set: {pos_freq}")
...
def train(args):
    ...
    output_directory = os.path.join("experiments", args.experiment_subdir)
    ...
    if type(all_annotations_df) is dict :
        if args.limited_data and len(all_annotations_df["train"]) > args.limited_data:
            if args.balance:
                logger.info("Balancing train set")
            # all_annotations_df["train"] = balance_df(all_annotations_df["train"], args.label_col, balance_ratio=args.balance_ratio)
                ...
                logger.info("Stratifed sampling of train set")
                _, all_annotations_df["train"] = train_test_split(all_annotations_df["train"], test_size=args.limited_data/len(all_annotations_df["train"]), stratify=all_annotations_df["train"][args.label_col], random_state=args.seed)
            else:
                logger.info("Random sampling of train set")
    ...
    if args.prev_model:
        ...
        prev_model_dir = os.path.join("experiments", args.prev_model)
        prev_model_path = os.path.join(prev_model_dir, "checkpoint-best")
        logger.info(f"loading prev model from {prev_model_path}")
        model = AutoModelForSequenceClassification.from_pretrained(prev_model_path, return_dict=True)

    else:    
        logger.info(f"loading model from {args.LM}")
    ...
    # Assuming res contains predictions and true labels
    predictions = res.predictions
    true_labels = res.label_ids

# Convert the predictions and true labels to a DataFrame
    df = pd.DataFrame({
            'labels': true_labels,
            'preds': predictions.argmax(axis=1)  # Assuming predictions is a softmax output
    })
    for i in range(predictions.shape[1]):
        df[f'logit_{i}'] = predictions[:, i]
    df.to_csv(os.path.join(output_directory, "test_preds.csv"))

    test_metrics = res.metrics
    test_metrics = {"test/"+k[5:]: v for k, v in test_metrics.items()}
    logger.info(f"logging test metrics to wandb {test_metrics}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stratify", action="store_true", help="limited data is sampled stratified or not")
    parser.add_argument("--prev_model", type=str, default=None, help="path to the prev experiment to load the best model from")
